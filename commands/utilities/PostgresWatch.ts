...
export namespace PostgresWatch {
    ...
    export async function createWatchTable(): Promise<void> {
        try {
            ...
            log(res, createWatchTable.name, "INFO");
        }
        catch (err) {
            log(err, createWatchTable.name, "ERROR");
        }
    }
    ...
    export async function searchCourse(user_id: string, course: string): Promise<CourseList> {
        try {
            ...
            log(res.rows, searchCourse.name, "INFO");

            return res.rows;
        }
        catch (err) {
            log(err, searchCourse.name, "ERROR");
        }
        ...
        try {
            const res = await pool.query(`
                SELECT user_id, 
                       ...
                       previously_open
                FROM rubot.watch;`);
            log(res.rows, getAllAlertCourses.name, "INFO");

            return res.rows;
        }
        catch (err) {
            log(err, getAllAlertCourses.name, "ERROR");
        }
    ...
    export async function insertClass(user_id: string, course: string, channel_id: string): Promise<boolean> {
        try {
            ...
            log(res, insertClass.name, "INFO");
            return true;
        }
        catch (err) {
            log(err, insertClass.name, "ERROR");
            return false;
        }
    }
    ...
    export async function updatePrevOpenCol(user_id: string, course: string, open: boolean): Promise<unknown[]> {
        try {
            ...
            log(res, dropCourseTable.name, "INFO");

            return res.rows;
        }
        catch (err) {
            log(err, dropCourseTable.name, "ERROR");
        }
        ...
        setInterval(async () => {
            ...
            for (const course of Object.keys(map)) {
                ...
                ScraperApiWrapper.getInstance().getCourseInfo(
                    process.env.CURR_TERM!, subject, num).then(async courseInfoList => {
                    ...
                    for (const section of courseInfoList) {
                        ...
                        if (section.available_seats > 0 && section.waitlist_ct <= 0 && section.is_visible) {
                            ...
                            for (const channelInfo of Object.keys(userMap)) {
                                ...
                                log(`Alerted ${userList.toString()}for ${course}`, "Alert sent", "INFO");
                            }
                            break;
                        }
