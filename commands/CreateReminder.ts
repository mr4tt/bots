...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const remindEmbed = generateBlankEmbed(interaction.user, "Green")
      ...
      .setFooter({
        text: `Server Context: ${
          interaction.guild?.name ?? "Direct Message @edbird"
        }`,
      })
      .setDescription(
        "Hello! You'd like to be reminded about: " +
          `${reminderMsg}` +
          "\nWhen setting a date, the hour is mandatory. \n- If the year / month / day is left blank, " +
          "it will default to the current year / month / day. \n- If the minute is left blank, it will default to 0."
      );
    ...
    while (true) {
      ...
      if (interact.isButton()) {
        if (interact.customId === `${uniqueId}_date`) {
          ...
          const modalSubmission = await interact.showModal(modal).then(() =>
            interact!.awaitModalSubmit({
          ...
          try {
            ...
            const minute =
              Number.parseInt(
                modalSubmission.fields.getTextInputValue("minute")
              ) || 0;
            ...
            if (!Number.isNaN(date.getTime())) {
              if (date < new Date()) {
                ...
                message =
                  "Date set! " +
                  `${`<t:${Math.trunc(date.getTime() / 1000)}:R>`}, ` +
